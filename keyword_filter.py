...
NAV_INTENT = (
    # '앱·어플'은 제외한다 — "블랙박스 어플 연결"처럼 사용법 질의의 핵심어라
    # 탐색형으로 묶으면 성과 유형을 통째로 차단하게 된다.
    r"(고객센터|채용|주가|공시|매장|지점|본사|대리점|서비스센터|닷컴|홈페이지|"
    r"로그인|회원가입|고객상담|콜센터|영업시간|위치|주소|전화번호|"
    r"베스트샵|직영점|플래그십)"
)
...
def reject_reason(kw):
    ...
    # 모델번호 차단은 해제한다. "파인뷰X1000", "AGM105배터리" 같은 제품 키워드가
    # 실측에서 가장 높은 CTR 을 내는 유형이었다. 순수 코드(RH9SG)는 아래
    # "한글 부족" 규칙이 이미 걸러낸다.
    if _re_bad.search(k):
        return "부적절"
